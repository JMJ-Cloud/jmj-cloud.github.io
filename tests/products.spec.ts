...
test('compass page renders with embedded demo and beta link', async ({ page }) => {
  await page.goto('/products/compass');
  await expect(page.getByRole('heading', { level: 1, name: 'JMJ Compass' })).toBeVisible();
  await expect(page.locator('iframe[src="/demos/compass/index.html"]')).toBeAttached();
...
test('products hub lists both tiers', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'JMJ Compass' })).toBeVisible();
