# Talk to Your ERP: How OIC Gen 3 MCP Servers Could Streamline Oracle ERP Administration

*What follows is a mix of what's available today in Oracle's stack and my own thoughts on where this could realistically be heading. Not all of this is turnkey right now, but the building blocks exist - and it's worth exploring what becomes possible when you connect them together.*
...
The orchestration pieces (OIC tools and an agent) are coming together quickly, but there's still a practical question: what is the conversational interface that users actually talk to, and how does that interface securely invoke the LLM and the tool layer?

There are a few viable patterns:

- **Inside Fusion:** If you're leaning into Oracle's application-native agent direction, the "front end" can be the Fusion experience itself via Agent Studio-style capabilities - inheriting Fusion identity, roles, and user context.
- **Outside Fusion:** If you want the agent where admins already work, you can put it in Teams/Slack (or a lightweight web UI). In that world, Oracle Digital Assistant becomes a credible option as a conversational front end that can integrate with OCI services and route intent to OIC tools - while also handling authentication, conversation UI, and integration with enterprise channels.

The key point is that the chat experience and the tool execution layer are separate concerns. Getting the interaction layer right - identity, authorization, approvals, and audit visibility - is often what determines whether this becomes a secure enterprise capability or stays a demo.
...
*Matt Paine is an Oracle ERP Cloud Solution Architect and owner of JMJ Cloud Inc., specializing in Oracle Fusion implementations, Oracle Integration Cloud development, and enterprise system integrations.*
